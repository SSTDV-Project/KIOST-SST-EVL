...
from model import LSTM, ExtremeValueLoss
...
if __name__ == '__main__' :
    ...
    model = LSTM(hidden_size=32, output_size=1, num_layers=2, memory_size=memory_size, time_step=time_step).cuda()
    ...
    for i in range(iteration):
        if i == 0:
            ...
            memory = (location, time[:, :time_step[0]], input[:, :time_step[0]], time[:, time_step[0]:], torch.zeros(memory_size, time_step[1], 1).cuda(), label + 1)

        input, time, label, location = Generate_Batch(x_array, x_label, batch_size, date_split, time_step)

        model.construct_memory(memory[0], memory[1], memory[2], memory[3], memory[4], memory[5])
