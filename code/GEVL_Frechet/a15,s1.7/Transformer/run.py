...
from model import Transformer, FrechetGEVL
...
if __name__ == '__main__' :
    ...
    model = Transformer(hidden_size=32, output_size=1, num_layers=2, time_step=time_step).cuda()
    ...
    for i in range(iteration):
        ...
        optimizer.zero_grad()
        loss = FrechetGEVL(model(location, time[:, :time_step[0]], sst[:, :time_step[0]], time[:, time_step[0]:], torch.zeros(batch_size, time_step[1], 1).cuda()), sst[:, time_step[0]:])
        loss.backward()
        optimizer.step()
